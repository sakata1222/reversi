...
public class ReversiApplication extends Application {
  ...
  private static final String ROOT_FXML = "reversi_javafx.fxml";
  ...
  public void start(Stage stage) throws Exception {
    URL p = getClass().getResource(ROOT_FXML);
    root = FXMLLoader.load(getClass().getResource(ROOT_FXML));
  ...
  void putPiece(ReversiPosition position, ReversiColor color) {
    if (pieces.containsKey(position)) {
      ...
      JavaFxPlatformAccessUtils.runSync(() -> root.add(piece, position.getX(), position.getY()));
